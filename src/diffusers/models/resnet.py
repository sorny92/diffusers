...
class ConvnextBlock2D(nn.Module):
    ...
    Parameters:
        ...
        conv_2d_out_channels (`int`, *optional*, default to `None`): the number of channels in the output.
            If None, same as `out_channels`.
        mid_channels_mult (`int`, *optional*, default to 2): the multiplier on the middle channels.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.in_channels = in_channels
        out_channels = in_channels if out_channels is None else out_channels
        mid_channels = out_channels * mid_channels_mult
        self.out_channels = out_channels
        self.use_conv_shortcut = conv_shortcut
        self.up = up
        self.down = down
        ...
        self.conv1 = torch.nn.Conv2d(in_channels, in_channels, kernel_size=7, stride=1, padding=3, groups=in_channels)

        if temb_channels is not None:
            if self.time_embedding_norm == "default":
                self.time_emb_proj = torch.nn.Linear(temb_channels, in_channels)
            elif self.time_embedding_norm == "scale_shift":
                self.time_emb_proj = torch.nn.Linear(temb_channels, 2 * in_channels)
        ...
        if self.time_embedding_norm == "ada_group":
            self.norm1 = AdaGroupNorm(temb_channels, in_channels, groups_out, eps=eps)
        else:
            self.norm1 = torch.nn.GroupNorm(num_groups=groups_out, num_channels=in_channels, eps=eps, affine=True)

        self.dropout = torch.nn.Dropout(dropout)
        self.conv2 = torch.nn.Conv2d(in_channels, mid_channels, kernel_size=1, stride=1, padding=0)
        ...
        conv_2d_out_channels = conv_2d_out_channels or out_channels
        self.conv3 = torch.nn.Conv2d(mid_channels, conv_2d_out_channels, kernel_size=1, stride=1, padding=0)
